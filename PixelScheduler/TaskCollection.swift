...
struct TaskCollection: Hashable, Codable {
    ...
    init(style: Style, title: String) {
        ...
        if style == .puzzle {
            ...
            self.puzzleConfig = nil
        }
        self.id = UUID()
    }
    
    // Dummy data
    static let  listCollection = TaskCollection(style: .list, title: "쇼핑 리스트 (샘플)")
    static let puzzleCollection = TaskCollection(style: .puzzle, title: "보고 싶은 영화 (샘플)")
}
