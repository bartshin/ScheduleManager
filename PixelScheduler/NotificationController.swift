...
class NotificationController {
	...
	func setAlarm(of newSchedule: Schedule, numberOfRepeatForEach: Int = 5, character: SettingKey.Character)  {
	...
	private func addAlarm(foreach dates: [Date], of schedule: Schedule, character: SettingKey.Character) {
		// Check permission
		let center = UNUserNotificationCenter.current()
		...
		do {
			...
			let scheduleDictonary = try JSONSerialization.jsonObject(with: json, options: .allowFragments) as! [String : Any]
			content.userInfo["schedule"] = scheduleDictonary
		}
		catch {
			assertionFailure("Fail to pass schedule information to notification \(schedule)")
		}
