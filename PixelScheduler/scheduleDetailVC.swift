...
class scheduleDetailVC: UIViewController, ColorBackground, PlaySoundEffect {
    ...
    @IBAction func tapAlarmButton(_ sender: UIButton) {
        ...
        if !modelController.replaceSchedule(schedulePresenting!, to: newSchedule, alarmCharacter: settingController.character) {
            let alert = UIAlertController(
                title: "변경 실패",
                message: "알람 변경을 실패하였습니다 설정에서 권한을 확인해주세요", preferredStyle: .alert)
            ...
            do {
              let results = try contactGather.getContacts(by: [contact.contactID], forImage: false)
                if let firstContact = results.first {
                    ...
                    presentAlert(with: "연락처를 찾을 수 없습니다")
                }
            }catch {
                assertionFailure("Fail to fetch contact \n" + error.localizedDescription)
                presentAlert(with: "데이터 오류로 연락처를 가져오지 못했습니다")
            }
        } deniedHandler: {
            presentAlert(with: "설정에서 연락처 권한을 허용해 주세요")
        }
    }
    ...
    @IBAction func tapCompleteButton(_ sender: UIButton) {
        ...
        func changeModel() {
            if modelController.replaceSchedule(schedulePresenting!, to: newSchedule, alarmCharacter: settingController.character) {
                ...
                let alert = UIAlertController(
                    title: "변경 실패",
                    message: "스케쥴 완료 상태를 변경하는데 오류가 발생했습니다 나중에 다시 시도해 주세요", preferredStyle: .alert)
                let dismissAction = UIAlertAction(title: "확인", style: .default)
                alert.addAction(dismissAction)
                present(alert, animated: true)
                return
            }
