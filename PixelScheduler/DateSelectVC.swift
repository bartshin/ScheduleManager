...
class DateSelectVC: UIViewController {
    ...
    // MARK: Controller
    let cyclePicker = UIHostingController(rootView: MultiSegmentView())
    ...
    var dateLanguage: SettingController.DateLanguage = .korean
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if recievedCycle != nil {
            cyclePicker.rootView.shared.selectedIndices = recievedCycle!.0 == .weekday ? Set(recievedCycle!.1.compactMap { $0 - 1 }): Set(recievedCycle!.1)
        }
    }
    ...
    func getDatePicked() -> Schedule.DateType? {
        ...
        case 2:
            return .cycle(since: spotDatePicker.date, for: .weekday, every: cyclePicker.rootView.shared.selectedIndices.compactMap{$0 + 1})
        case 3:
            return .cycle(since: spotDatePicker.date, for: .day, every: cyclePicker.rootView.shared.selectedIndices.compactMap{$0})
        default:
            return nil
        }
    }
