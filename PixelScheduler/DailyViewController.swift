...
//		}
	...
	override func viewWillDisappear(_ animated: Bool) {
		...
		if navigationController?.modalPresentationStyle == .fullScreen,
			 ...
			 let calendarVC = tabBarController.viewControllers!.first(where: { $0 is CalendarViewController}) as? CalendarViewController {
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if segue.identifier == SegueID.TopViewControllerSegue.rawValue{
			...
			let now = Date()
			var dateComponents = Calendar.current.dateComponents([.year, .month, .day], from: dateShowing)
			dateComponents.hour = now.hour
			dateComponents.minute = now.minute
			editScheduleVC.recievedDate = Calendar.current.date(from: dateComponents) ?? dateShowing
