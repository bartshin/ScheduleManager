...
class ListTaskCell: UITableViewCell, PlaySoundEffect {
    ...
    @IBOutlet private weak var bulletinBoard: UIImageView!
    @IBOutlet private weak var distantCharterObject: NSLayoutConstraint!

 
    
    
    var character: SettingController.Character!
        ...
        firstAnimator.addCompletion { [self] progress in
            if progress == .end {
                ...
                if character == .wizard || character == .dragon {
                    bulletinBoard.loadGif(name: character.magicGif!)
                }else if character == .goblin {
                    bulletinBoard.image = character.arrowImage
                }
                bulletinBoard.isHidden = false
                bulletinBoard.layer.transform = CATransform3DTranslate(CATransform3DIdentity, -200, 0, 0)
                let secondAnimator = UIViewPropertyAnimator(
                    duration: 0.5,
                    curve: .linear) { [weak weakSelf = self] in
                    if weakSelf != nil {
                        weakSelf!.bulletinBoard.layer.transform = CATransform3DIdentity
                    }
                }
    ...
    private func updateUI(){
        titleLabel.text = task!.completed ? task!.text + " (완료)": task!.text
        bulletinBoard.isHidden = true
